...
# source https://github.com/broadinstitute/warp/blob/e6330f1fda1b738ea99e9728b6fa1b5d9b6ba530/beta-pipelines/skylab/ATAC/ATAC.wdl
# changes (almost all needed for miniwdl as well):
# 1. made all monitoring_logs optional (error in the original?)
# 2. BWAPairedEndAlignment: let the removal of the tar_bwa_reference fail without failing the step
# 3. FilterMitochondrialReads: localized bam_input to that it can be indexed
# 4. SnapCellByBin: cp, don't move the input file
...
task FilterMitochondrialReads {
  ...
  command {
    ...
    mkdir bam_input
    ln -s ~{bam_input} bam_input/~{basename(bam_input)}
    samtools index -b bam_input/~{basename(bam_input)}

    #get list of chromosomes from bam header (ignoring chrM chromosome)
    declare -r LIST_CHRS=`samtools view -H bam_input/~{basename(bam_input)} \
      | grep chr \
      | cut -f2 \
      | sed 's/SN://g' \
      | grep -v 'chrM\|_'`

...
inputs:
  ...
  - id: readsDownstream
    ...
    default: []
    type:
      name: _readsDownstream_File_array
      items: File
      type: array
  - id: outputPath
  ...
  - id: samRG
    ...
    type:
      ...
      - 'null'
  - id: picardXmx
    doc: The maximum memory available to the picard (used for sorting the 
      output). Should be lower than `memory` to accommodate JVM overhead and 
      bowtie's memory usage.
    default: 4G
    type: string
  - id: threads
...
baseCommand:
  ...
  - script.bash
arguments:
  - valueFrom: ${if (inputs.readsUpstream.length == 0) {throw "readsUpstream 

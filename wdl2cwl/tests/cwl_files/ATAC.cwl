...
id: ATAC
class: Workflow
doc: Processing for single-cell ATAC-seq data from the level of raw fastq reads 
  to the generation of a snap file with snaptools. ATAC-seq (Assay for 
  Transposase-Accessible Chromatin using sequencing) is a technique used in 
  molecular biology to assess genome-wide chromatin accessibility. This pipeline
  accepts fastq files where the cell barcode has been added to the fastq read 
  names as the first field.
requirements:
  - class: InlineJavascriptRequirement
  - class: StepInputExpressionRequirement
inputs:
  - id: fastq_gzipped_input_read1
    doc: read 1 fastq file as input for the pipeline, the cellular barcodes must
      be the first part of the read name seperated by colon
    type: File
  - id: fastq_gzipped_input_read2
    doc: read 2 fastq file as input for the pipeline, the cellular barcodes must
      be the first part of the read name separated by colon
    type: File
  - id: min_length
    doc: minimum length for trimming. Reads that are too short even before 
  ...
  - id: tar_bwa_reference
    doc: the pre built tar file containing the reference fasta and corresponding
  ...
  - id: bwa_cpu
    ...
    type: int
  - id: genome_name
    doc: the name of the genome being analyzed, input to snap tools, curently 
      mm10 and hg19 supported
    type: string
  - id: genome_size_file
    doc: name of the file with chromosome sizes for the genome in the input tar 
      file
    type: File
  - id: min_map_quality
    doc: the minimum mapping quality to be filtered by samtools view and 
      snap-pre (snaptools task)
    type: int
  - id: max_fragment_length
    doc: the maximum fragment length for filtering out reads by gatk and 
      snap-pre (snaptools task)
    type: int
  - id: output_base_name
...
steps:
    ...
    run:
      ...
      inputs:
        ...
        - id: fastq_input_read2
          ...
          type: File
        - id: min_length
          doc: the minimum legnth for trimming. Reads that are too short even 
            before adapter removal are also discarded
          type: int
        - id: quality_cutoff
          doc: cutadapt option to trim low-quality ends from reads before 
            adapter removal
          type: int
        - id: adapter_seq_read1
          doc: cutadapt option for the sequence adapter for read 1 fastq
    ...
    run:
      ...
      inputs:
        ...
        - id: fastq_input_read2
          ...
          type: File
        - id: tar_bwa_reference
          doc: the pre built tar file containing the reference fasta and 
            cooresponding reference files for the BWA aligner
          type: File
        - id: read_group_id
          doc: the read group id to be added upon alignment
          type: string
    ...
    run:
      ...
      inputs:
        - id: bam_input
          ...
          type: File
        - id: max_fragment_length
          doc: the maximum fragment length for filtering out reads by gatk 
            (snaptools task)
          type: int
        - id: output_base_name
          doc: base name to be used for the output of the task
        ...
        - id: bam_filter_fragment_length_output
          ...
          outputBinding:
            glob: $(inputs.output_base_name + 
              ".filtered.max_fragment_length.bam")
        - id: monitoring_log
          type:
            - File
            - 'null'
    ...
    run:
      ...
      outputs:
        - id: bam_no_chrM_reads_output
          ...
          outputBinding:
            glob: $(inputs.output_base_name + 
              ".filtered.no_mitochondrial_reads.bam")
        - id: bam_chrM_reads_output
          type: File
          outputBinding:
            glob: $(inputs.output_base_name + 
              ".filtered.mitochondrial_reads.bam")
        - id: monitoring_log
          type:
            - File
            - 'null'
  ...
  - id: MakeCompliantChrMBAM
    ...
    run:
      ...
      inputs:
        - id: bam_input
          doc: the bam with barcodes in the read ids that need to be converted 
            to barcodes in bam tags
          type: File
        - id: output_base_name
          doc: base name to be used for the output of the task
          type: string
    ...
    run:
      ...
      inputs:
        ...
        - id: genome_name
          ...
          type: string
        - id: max_fragment_length
          doc: the maximum fragment length for filtering out reads by snap-pre 
            (snaptools task)
          type: int
        - id: genome_size_file

...
inputs:
  ...
  - id: inputBamIndex
    ...
    type: File
  - id: modelType
    doc: <WGS|WES|PACBIO>. Type of model to use for variant calling. Each 
      model_type has an associated default model, which can be overridden by the
      --customized_model flag.
    type: string
  - id: outputVcf
    doc: Path where we should write VCF file.
    type: string
  - id: postprocessVariantsExtraArgs
    doc: A comma-separated list of flag_name=flag_value. 'flag_name' has to be 
      valid flags for calpostprocess_variants.py.
    type:
      - string
      - 'null'
  - id: customizedModel
    doc: A path to a model checkpoint to load for the `call_variants` step. If 
      not set, the default for each --model_type will be used.
    type:
      - File
      - 'null'
  - id: numShards
  ...
  - id: outputGVcf
    ...
    type:
      ...
      - 'null'
  - id: outputGVcfIndex
    doc: Path to where the gVCF index file will be written. This is needed as a 
      workaround, set it to `outputGVcf + '.tbi.'`
    type:
      - string
      - 'null'
  - id: regions
    ...
    type: string
  - id: sampleName
    doc: Sample name to use instead of the sample name from the input reads BAM 
...
outputs:
  ...
  - id: outputVCFStatsReport
    doc: Statistics file.
    type:
      name: _outputVCFStatsReport_File_array
      items: File
      type: array
    outputBinding:

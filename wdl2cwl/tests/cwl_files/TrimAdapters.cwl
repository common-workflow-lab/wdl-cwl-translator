...
hints:
  ...
  - class: ResourceRequirement
    ...
    outdirMin: $((Math.ceil(2 * 
      (function(size_of=0){inputs.fastq1_input_files.forEach(function(element){ 
      if (element) {size_of += element.size}})}) / 1024^3 + 
      (function(size_of=0){inputs.fastq2_input_files.forEach(function(element){ 
      if (element) {size_of += element.size}})}) / 1024^3)  + 10) * 1024)
inputs:
  - id: fastq1_input_files
    type:
      name: _fastq1_input_files_File_array
      items: File
      type: array
  - id: fastq2_input_files
    type:
      name: _fastq2_input_files_File_array
      items: File
      type: array
  - id: adapter_list
    type: File
  - id: input_ids
    type:
      name: _input_ids_string_array
      items: string
      type: array
  - id: docker
    doc: (optional) the docker image containing the runtime environment for this
      task
  ...
  - id: disk
    ...
    type:
      ...
      - 'null'
  - id: preemptible
    doc: (optional) if non-zero, request a pre-emptible instance and allow for 
      this number of preemptions before running the task on a non preemptible 
      machine
    default: 3
    type: int
baseCommand:
...
outputs:
  - id: trimmed_fastq1_files
    type:
      name: _trimmed_fastq1_files_File_array
      items: File
      type: array
    outputBinding:
  ...
  - id: trimmed_fastq2_files
    type:
      name: _trimmed_fastq2_files_File_array
      items: File
      type: array
    outputBinding:
